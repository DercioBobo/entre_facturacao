"""Extrato com IVA: uma linha por factura do cliente, com o valor líquido, o
IVA, o valor total da factura, o valor já pago (somando todas as alocações de
pagamento recebidas para essa factura), o saldo pendente e o estado."""
...
def get_columns():
	return [
		...
		{"label": _("Valor Líquido"), "fieldname": "valor_liquido", "fieldtype": "Currency", "width": 120},
		{"label": _("IVA"), "fieldname": "iva", "fieldtype": "Currency", "width": 100},
...
def get_facturas(cliente, empresa=None):
	...
	rows = frappe.db.sql(
		f"""
		SELECT si.name, si.posting_date, si.due_date, si.net_total, si.total_taxes_and_charges,
			si.grand_total, si.outstanding_amount, si.invoice_title,
			COALESCE(SUM(per.allocated_amount), 0) AS valor_pago
		...
		linhas.append(
			{
				...
				"valor_liquido": flt(r.net_total),
				"iva": flt(r.total_taxes_and_charges),
